...
class TestNdsPVTimestamps(ParametrizedTestCase):
    ...
    def setUp(self):
        try:
            ...
            appName = 'ndsex1'
            libFile = 'libnds3-DeviceVectorDBL.so' 
            subsFile = 'WaveForm.substitutions'
            DeviceName = "B-0"
            DriverName = "DeviceVectorDBL"
            ...
            print(self.line)
            print("|")
            print("|                TEST Timestamping provided by NDS Device Driver")
            print("|")
            print(self.line)    
            print("|")
            ...
            self.prefix = CBS1+'-'+CBS2\
                +'-HWCF:'+BOARDTYPE+'-'+BOARDTYPEIDX+'-'+WFG_NAME    
    ...
    def testState(self):
        ...
        try:   
            sleep(1)
            dataPV = self.prefix+'-Data'
            frequencyPV = self.prefix+'-Frequency'
            frequencyPV_RBV = self.prefix+'-Frequency_RBV'
            nameG = self.prefix+'-STM-getState'
            nameS = self.prefix + '-STM-setState'
            nameGG = self.prefix + '-STM-getGlobalState'
            ...
            epics.caput(frequencyPV,0,wait= False,timeout=5)
            epics.caget(frequencyPV_RBV)
            epics.caget(dataPV)
            print dataPV + ' timestamp TSE==0: ' +epics.pv.fmt_time(epics.pv.get_pv(dataPV).timestamp)
            print frequencyPV_RBV + ' timestamp TSE==0: ' +epics.pv.fmt_time(epics.pv.get_pv(frequencyPV_RBV).timestamp)
            
            ''' We set the TSE field to -2 in the Data PV'''
            epics.caput(dataPV + ".TSE",-2,wait= False,timeout=5)
            epics.caput(frequencyPV_RBV + ".TSE",-2,wait= False,timeout=5)
            
            sleep(1)
            epics.caput(frequencyPV,1,wait= False,timeout=5)
            ...
            epics.caget(dataPV)
            value = epics.pv.get_pv(dataPV).timestamp
            self.assertEqual(value,NDS_EPOCH)
            value=epics.pv.fmt_time(epics.pv.get_pv(dataPV).timestamp)
            print dataPV+' timestamp TSE==-2: ' + value
            ...
            self.assertEqual(value,NDS_EPOCH) 
            value =epics.pv.fmt_time(epics.pv.get_pv(frequencyPV_RBV).timestamp)
            print frequencyPV_RBV+' timestamp TSE==-2: ' + value
            sleep(1)
